...
p.caption {
  font-size: 0.8em;         /* This sets figure caption font size */
  text-align: left
}
...
This paper explored patterns of missing data by indices of readability and item desirability using both self and informant ratings. 

The hypotheses below were explored in three stages: 1) examining the most recent and uniquely apt General Social Survey data for patterns in the reasons for non-response, 2) exploring overall readability and social desirability as they relate to missing responses, and 3) considering potential differences in missing responses across self-, self-nominated other, and non-self-nominated other ratings. 
...
Data from three sources were used to explore the above predictions: 1) 2020 data from the General Social Survey (GSS), 2) ratings of item social desirability, and 3) personality ratings from undergraduates.  
...
For the 2020 GSS data, across 5,215 total responses, 0.19% were missing when respondents reported “don’t know” and 0.09% were missing because respondents refused to answer (i.e., “no answer), or 0.17% skipped responses on the web supporting `H1`.

Directly comparing the qualitative item characteristics with ratings of socially desirable content, we applied logistic regression, treating missed and non-missed responses as binary outcomes. In this approach, the strongest standardized predictors were the Dale-Chall ($b = -.03, z = -6.63, p < .001$) and item social desirability ($b = -.15, z = -6.23, p < .001$). The Fog-NRI was the only non-significant predictor of missingness ($b = .37, z = 1.91, p = .060$). Results are supportive of `H2`, `H3` and partially in support of `H4` (presented visually in Figure \@ref(fig:img-with-knitr)).
...
A simple regression revealed that frequency of missing responses decreased as social desirability increased ($F_(1,22) = 6.77, p = .016$), also supporting `H3`, see  Figure \@ref(fig:FigureNEW))
...
Using only summaries of missingness within social desirability stratum, as well as an indication of source (self, self-nominated, or non-self-nominated), the overall $R^2$ was .89 $(F(5,18) = 28.18, p < .001)$.There were strong main effects for social desirability stratum $(F(1,22) = 6.77, p = .016)$ as well as rater type $(\Delta F(2,20) = 42.49, p < .001)$. The interaction effect (rater type x social desirability stratum) was non-significant $(\Delta F(2,18) = 2.57, p = .100)$. This is partial support of `H5`, see Figure \@ref(fig:Figure4). These results are in contrast with the expectation that self-nominated rating sources would possibly be more influenced by item desirability (“skip” a larger number of undesirable items).

```{r Figure4, echo=FALSE, warning=FALSE, message=FALSE, results=FALSE, fig.width=11, fig.height=6, fig.cap="Smoothed (regression-predicted) missing value estimates across social desirability strata and rater type.", fig.height=7}
